...
module.exports = function createProjection (schemaName) {
  ...
  async function declineTransfer (pgClient, data) {
    ...
    const result = await pgClient.query(`SELECT to_id FROM ${schemaName}.transaction WHERE id = $1`, [transactionId])
  ...
  async function completeTransfer (pgClient, data) {
    ...
    const result = await pgClient.query(`SELECT from_id FROM ${schemaName}.transaction WHERE id = $1`, [transactionId])
